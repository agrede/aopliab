...
tmp = np.load(svepth+"genoise.npz")
...
for k, l in enumerate(lamGe):
    ...
    if l > fw.next_filter_on*1.02:
        if fw.position == 6:
            ...
            fw.position = fw.position + 1
    meassGe[k, :] = meas()
    phc = float(l)*meassGe[k, 0]/(GeR([l])[0]*hc)
    p.update(l, np.abs(phc))
    respGe[k, :] = np.hstack(([phc], meassGe[k, [2, 4]]/phc))
...
rSi = interp1d(lamC, respC[:, 0]/1.9881402957705074, kind='cubic')
...
for k, l in enumerate(lam):
    ...
    np.savez_compressed(svepth+"14nmcSi_pos22_5VNewObj", lam, meass, deqe)
