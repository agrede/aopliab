...
class Keysight2900:
    ...
    '''four_point: enables or disables 4/four-wire sensing
    variables...
        ch = channel #
        value = 1 for 4 point, any other value is 2 point'''
    def four_point(self, ch, value):
        if value == 1:
...
#            print("Nothing happens with neither channel selected, dufus.")
 ...
 ########### Sweep #########           
    ...
    def sweep_bidirectional(self, ch, value):
        if value == 1:
            self.inst.write("SOUR%s:SWE:STA DOUB" % ch)       
        else:
    ...
    ***IMPORTANT*** This assumes that ch2 is set at DC and only ch 1 is being pulsed'''
    ...
    def dc_internal_IV(self, bias_start, bias_stop, bias_step, current_limit, compliance_protection,
                       int_time, timeout, two_or_four_pt, bidirectional):
        ...
        self.sweep_bidirectional(1, bidirectional)
...
class Keysight2900b:
    ...
    def output_over_protection(self, ch, value):
        if value == 1:
            self.inst.write("OUTP%d:PROT ON" % ch)
        else:
            self.inst.write("OUTP%d:PROT OFF" % ch)
...
#            print("Nothing happens with neither channel selected, dufus.")
 ...
 ########### Sweep #########           
    ...
    '''Sets source wait time. value 0 off, 1 on'''
    def source_wait(self, ch, value):
        if value == 0:
            self.inst.write("SOUR%d:WAIT:AUTO % (ch, value)")
        if value == 1:
            self.inst.write("SOUR%d:WAIT:AUTO % (ch, value)")
    ...
    ***IMPORTANT*** This assumes that ch2 is set at DC and only ch 1 is being pulsed'''
    ...
    ''' dc_internal_IV - performs internal voltage sweep for a single channel'''    
    def dc_internal_IV(self, bias_start, bias_stop, bias_step, current_limit, compliance_protection,
                       int_time, timeout, two_or_four_pt, bidirectional, ch):
        
        #set ch 1 to source voltage, ch 2 to only read current w/ 100 mA compliance
        self.compliance(ch, 0, current_limit)
        self.source_auto_range(ch, 0, 1)
        
        #Set up sense subsystem for integration, sensitivity, etc.
        self.integration_time(ch,int_time)
        self.sense_measurements(ch, 1, 1, 1)
        self.sense_range_auto(ch, 0, 1)
        self.sense_range_auto(ch, 1, 1)
        self.sense_range_auto_llim(ch, 1, "MIN")
        self.sense_range_auto_llim(ch, 0, "MIN")
        self.sense_range_auto(ch, 2, 1)
        self.four_point(ch, two_or_four_pt)
        
        self.source_auto_range(1,0,1)        
        self.source_pulse(ch, 0)
        self.set_sweep_direction(ch, 0)   #sweeps from low to high
        self.source_sweep(ch, 0, 1)
        self.source_sweep_range(ch, 0, bias_start, bias_stop)
        self.source_sweep_step(ch, 0, bias_step)
        self.sweep_bidirectional(ch, bidirectional)
        ...
        if bidirectional == 1:
            npts = npts*2
        self.trigger_setup_dc(ch, npts)

        #Initiate both channels and measure data
        self.output_enable(ch,1);
        dat = self.measurement_all(ch,0, timeout)
        print(dat)
        dat = np.reshape(dat,[np.size(dat)//6,6])
        VIR = np.array( [dat[:,0], dat[:,1], dat[:,2]] ).T #return voltage | current | resistance
    ...
    def dc_internal_LIV(self, bias_start, bias_stop, bias_step, current_limit, photocurrent_limit, 
        ...
        #Set arm both channels and set trigger delays to 0
        npts = 2*abs((bias_stop - bias_start)/bias_step) + 2
        self.trigger_setup_dc(1, npts)
        self.trigger_setup_dc(2, npts)
